...
export class PipelineCoordinator {
    ...
    async fetchAndDecompressSession() {
        try {
            ...
            const canvasData = await api.fetchSessionCanvas(this.sessionId);
        ...
        for (const conn of outgoing) {
            ...
            try {
                ...
                if (targetNode.type === 'COMPONENT_GENERATE') {
                    ...
                    const rawResponse = await api.generateComponents(reqBody);
                    ...
                    const authVectors = await api.generateVectors(generatedComponents.id);
                ...
                if (targetNode.type === 'FORMAL_VERIFY') {
                    ...
                    const componentHolderNode = nodesArray.find(n => 
                    ...
                    const scenarioJson = await api.generateScenarios(indexId, authVecId);
                ...
                if (targetNode.type === 'TEST_GENERATE') {
                    ...
                    const promptNode = nodesArray.find(n => 
                    ...
                    const data = await api.generateTestSuites(selectedLlm, prompts); 
                ...
                if (targetNode.type === 'PROMPT_GENERATE') {
                    ...
                    const data = await api.generatePrompts(selectedIds, targetLanguage); 
                ...
                if (targetNode.type === 'AEGIS') {
                    ...
                    api.analyzeAegis(enginePayload)
                    .then(async (response) => {
                        if (!response.ok) {
                ...
                if (targetNode.type === 'CHANGE_IMPACT') {
                    ...
                    const result = await api.analyzeDelta(deltaInput); 
                ...
                if (targetNode.type === 'SECURITY_REGRESSION') {
                    ...
                    const [baseNode, prNode] = fvNodes.sort((a, b) => 
                        (a.position?.y ?? a.y ?? 0) - (b.position?.y ?? b.y ?? 0)
                    );
